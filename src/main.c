...
int TestBase64Encode(char* hexStr) {
...
int TestFixedXOR(char* hexStr1, char* hexStr2) {
...
int TestSingleXORcipher(char* hexStr) {
    ...
    unsigned char key;
    singleXOR_attack(byteBuf, byteBuf_l, &key);
    printf("Found key: 0x%02X ('%c')\n", key, key);

    unsigned char out[256];
    for (int i = 0; i < byteBuf_l; ++i) {
        out[i] = byteBuf[i] ^ key;
    }
    out[byteBuf_l] = 0;

    ascii_correct(out, byteBuf_l);
    printf("%s\n", out);
...
int main(int argc, char **argv)  {
    TestBase64Encode("49276d206b696c6c696e6720796f757220627261696e206c696b65206120706f69736f6e6f7573206d757368726f6f6d");
    TestFixedXOR("1c0111001f010100061a024b53535009181c", "686974207468652062756c6c277320657965");
    TestSingleXORcipher("1b37373331363f78151b7f2b783431333d78397828372d363c78373e783a393b3736");
